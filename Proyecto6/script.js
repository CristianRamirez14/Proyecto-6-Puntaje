...
document.querySelectorAll(".item").forEach(item =>{
    item.addEventListener("mouseover", (e) => {
        ...
        document.querySelectorAll(".item").forEach(cuadritoGris => {
            if (cuadritoGris.classList.contains("selec")) {
                cuadritoGris.classList.remove("selec");
            }
        });

        for (let i = 0; i <= posicion; i++) {
            const cuadrito = document.querySelector(`.item-${i}`);
                if (!cuadrito.classList.contains("selec")) {
